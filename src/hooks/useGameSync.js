...
export function useGameSync(pin) {
  ...
  const debounceRef = useRef({ players: null, answers: null });
  ...
  const refetchSessionPlayersAndAnswers = useCallback(
    ...
    [scheduleSetAnswers, scheduleSetPlayers]
  );
  ...
  return useMemo(
    () => ({ sessionData, players, answers, loading }),
    [answers, loading, players, sessionData]
  );
}
